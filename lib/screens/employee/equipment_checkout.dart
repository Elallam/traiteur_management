...
class _EquipmentCheckoutScreenState extends State<EquipmentCheckoutScreen>
  ...
  List<String> _categories = ['All']; // Initialize with default value

  @override
  void initState() {
    super.initState();
    // Initialize tab controller immediately with safe defaults
    _tabController = TabController(
      length: _categories.length,
      vsync: this,
    );

    // Load equipment immediately
    _loadEquipment();

    // Initialize categories after first frame
  ...
  Future<void> _sendCheckoutRequestNotification(
    ...
    try {
      // Prepare equipment list for notification
      List<Map<String, dynamic>> equipmentList = requests.map((r) => {
        'equipmentId': r.equipmentId,
        'equipmentName': r.equipmentName ?? 'Unknown Equipment',
        'quantity': r.quantity,
      }).toList();

      // Send enhanced notification (both in-app and push)
      await _notificationService.sendEquipmentCheckoutNotification(
      ...
      print('Error sending checkout request notification: $e');
      // Don't throw here to avoid disrupting the checkout process
    }
  }
